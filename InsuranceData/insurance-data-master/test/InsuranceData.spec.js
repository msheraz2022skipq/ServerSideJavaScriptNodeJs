...
describe("Testing read file and filter data", function () {
  ...
  it("Testing filtering of data with no childs", function (done) {
    let filteredData = [
      '38, male, 19.3, 0, yes, southwest, 15820.699',
      '39, female, 19.3, 2, no, southwest, 18820.699',
      '40, female, 19.3, 0, yes, northwest, 16000',
    ...
    InsuranceData.filterDataWithNoChildren(filteredData, (err, result) => {
      expect(err).to.be.equal(null);
      expect(result.length).to.be.equal(2);
      expect(result[0]).to.be.equal('38, male, 19.3, 0, yes, southwest, 15820.699');
      done(err);
    });
  });
